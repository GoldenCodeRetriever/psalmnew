...
def list_model_modules(model, prefix="model 的一级模块"):
    print(f"===== {prefix} =====")
    modules_info = []
    for name, module in model.named_modules():
        if name != "" and "." not in name:
            module_type = module.__class__.__name__  
            modules_info.append((name, module_type))
    
    modules_info.sort(key=lambda x: x[0])
    
    for name, module_type in modules_info:
        print(f"- {name} (类型: {module_type})")
    print(f"===== 共 {len(modules_info)} 个一级模块 =====")
...
def count_model_parameters(model):
    ...
    for name, param in model.named_parameters():  
        ...
        if param.requires_grad:
            ...
            frozen_params += param_num
    
    print(f"{total_params:,} 个({total_params/1e6:.2f}M)")
    print(f"{trainable_params:,} 个({trainable_params/1e6:.2f}M)")
    print(f"{frozen_params:,} 个({frozen_params/1e6:.2f}M)")
...
class ModelArguments:
    ...
    vision_tower_train_backbone: bool = field(default=False)
    ...
    cross_image_seg_task: bool = field(default=False)
...
class DataArguments:
    ...
    referring_image_folder: Optional[str] = "/share/zhangyudong6-local/mission_1_track1_test/s"
    region_cross_image_folder: Optional[str] = "/share/zhangyudong6-local/mission_1_track1_test/s"
    region_image_folder: Optional[str] = "/share/zhangyudong6-local/mission_1_track1_test/s"
    region_mask_type: str = 'box_visual_prompt_mask||scribble_visual_prompt_mask||point_visual_prompt_mask'
    ...
    region_cross_json_path: str = '/path/to/visual_prompt_segmentation_train.json'
    panoptic_json_path: str = "/path/to/coco"
    referring_json_path: str = "/share/zhangyudong6-local/PSALM/output/test1/referring.json"
...
def make_unify_datamodule(tokenizer, data_args, training_args, seg_task, cross_image_seg_task):
    datasets = []
    ...
    # panoptic_coco_dataset = COCO_panoptic_dataset_random(json_path=data_args.panoptic_json_path, tokenizer=tokenizer,data_args=data_args)
    # datasets.extend([panoptic_coco_dataset]*data_ratio[0])
    ...
    referring__dataset = referring_dataset(json_path=data_args.referring_json_path, tokenizer=tokenizer, data_args=data_args)
    ...
    train_dataset = UnifyDatasetSingleDatasetForBatch(datasets,data_ratio,8,fix_dataset_len=data_args.fix_dataset_len)
...
def train():
    ...
    print(f"当前使用的梯度裁剪值 (max_grad_norm): {training_args.max_grad_norm}")
    ...
    list_model_modules(model=model)
    ...
    # 若tokenizer无pad_token，添加[PAD]并调整嵌入层
    ...
    if model_args.vision_tower is not None:
        ...
        if not model_args.vision_tower_train_backbone:
            model.model.vision_tower.requires_grad_(False)
            print(f"不训练视觉编码器backbone,冻结视觉塔")
    ...
    # print(torch.cuda.memory_summary(device=training_args.device, abbreviated=False))

    # resume_path = "/home/zhangkunquan/code/PSALM/output/checkpoint/RRSIS-D_SIOR_10/checkpoint-21510"
    # if not os.path.exists(resume_path):
    #     raise FileNotFoundError(f"手动指定的权重路径不存在：{resume_path}")
    # print(f"===== 加载手动指定的权重路径续训：{resume_path} =====")
