...
class interactive_dataset(COCO_panoptic_dataset):
    def __init__(self, json_path, tokenizer, data_args):
        if isinstance(json_path, list):
            data = []
            for path in json_path:
        ...
        # ]
        # category_names = [
        #     'Ship', 'Harbor', 'Expressway toll station', 'Ground track field', 'Expressway service area', 
        #     'Train station', 'Golf field', 'Dam', 'Basketball court', 'Tennis court', 'Baseball field', 
        #     'Wind mill', 'Stadium', 'Storage tank', 'Vehicle', 'Overpass', 'Bridge', 'Chimney', 
        #     'Airplane', 'Airport'
        # ]
    ...
    def __getitem__(self, idx):
        ...
        image_folder = self.data_args.region_image_folder
        ...
        for annotation in data_dict['annotations']:
            ...
            if annotation['category_id'] in self.coco_id_to_cont_id:
                ...
                # print(f"category_id={original_category_id}, type={type(original_category_id)}")
                # keys = list(self.coco_id_to_cont_id.keys())
                # values = list(self.coco_id_to_cont_id.values())
                # print(f"映射键: {keys}")
                # print(f"映射值: {values}")
                # in_keys = original_category_id in keys
                # in_values = original_category_id in values
                # print(f"在键中: {in_keys}")
                # print(f"在值中: {in_values}")
                raise ValueError
        ...
        data_dict['input_ids'] = input_ids
        data_dict['labels'] = labels
        data_dict['dataset_type'] = 'region_seg'
        return data_dict
    

class Cross_interactive_dataset(COCO_panoptic_dataset):
    def __init__(self, json_path, tokenizer, data_args):
        if isinstance(json_path, list):
            data = []
            for path in json_path:
    ...
    def tokenizer_special_tokens(self, prompt, tokenizer, image1_token_index=IMAGE1_TOKEN_INDEX,
        ...
        prompt_chunks = re.split('(<image1>|<image1_deform>|<seg>|<cls>|<region>)', prompt)
...
class instance_dataset(interactive_dataset):
    def __init__(self, json_path, tokenizer, data_args):
        if isinstance(json_path, list):
            data = []
            for path in json_path:
...
class referring_dataset(instance_dataset):
...
class DataCollatorForCOCODatasetV2(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        #     print(f"  Content of x['instances1']:", seg['instances'])
        #     print(f"  Content of x['instances1']:", seg['instances1'])
        ...
        fields_to_delete = ['input_ids', 'labels', 'image']
        if 'image1' in instances[0]:
            fields_to_delete.append('image1')
        ...
        # # ===== 添加调试日志 =====
        # print("\n" + "="*50)
        # print("DEBUG: batch['seg_info'] 内容分析")
        # print("="*50)
        

        # print(f"seg_info 包含 {len(batch['seg_info'])} 个样本")

        # for i, seg in enumerate(batch['seg_info']):
        #     print(f"\n--- 样本 {i} 的键: {list(seg.keys())} ---")
        #     print(f"  Content of x['instances1']:", seg['instances1'])
            
        #     if 'instances' in seg:
        #         instances_info = seg['instances']
        #         print(f"  instances 类型: {type(instances_info)}")
        #         if hasattr(instances_info, '__dict__'):
        #             print(f"  instances 属性: {list(instances_info.__dict__.keys())}")
        #         if hasattr(instances_info, 'gt_masks'):
        #             print(f"  instances.gt_masks 形状: {seg['instances'].gt_masks.shape}")
            
            # if 'instances1' in seg:
            #     instances1_info = seg['instances1']
            #     print(f"  instances1 类型: {type(instances1_info)}")
            #     if hasattr(instances1_info, '__dict__'):
            #         print(f"  instances1 属性: {list(instances1_info.__dict__.keys())}")
            #     if hasattr(instances1_info, 'gt_masks'):
            #         print(f"  instances1.gt_masks 形状: {seg['instances1'].gt_masks.shape}")
            
        #     important_fields = ['dataset_type', 'class_name_ids', 'cls_indices', 'token_refer_id']
        #     for field in important_fields:
        #         if field in seg:
        #             value = seg[field]
        #             if torch.is_tensor(value):
        #                 print(f"  {field}: 张量, 形状: {value.shape}")
        #             else:
        #                 print(f"  {field}: {type(value)} - {value}")
        
        # for i, seg in enumerate(batch['seg_info']):
        #     if 'file_name' in seg:
        #         print(f"样本 {i} file_name: {seg['file_name']}")
        #     if 'file_name1' in seg:
        #         print(f"样本 {i} file_name1: {seg['file_name1']}")
