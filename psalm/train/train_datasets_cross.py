...
class Cross_interactive_dataset(COCO_panoptic_dataset):
    ...
    def tokenizer_special_tokens(self, prompt, tokenizer, image_token_index=IMAGE_TOKEN_INDEX,
                                 ...
                                 region_token_index=REGION_TOKEN_INDEX, return_tensors=None):
        input_ids = []
        special_token_map = {'<image>': image_token_index, '<image1>': image1_token_index, '<seg>': seg_token_index, '<cls>': cls_token_index, '<region>':region_token_index}
        prompt_chunks = re.split('(<image>|<image1>|<seg>|<cls>|<region>)', prompt)
...
class COCO_instance_dataset(interactive_dataset):
    def __init__(self, json_path, tokenizer, data_args):
        if isinstance(json_path, list):
            data = []
            for path in json_path:
...
class referring_dataset(COCO_instance_dataset):
