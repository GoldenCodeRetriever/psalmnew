...
class DataArguments:
    ...
    json_path: str = '/nfs-data1/zhangkunquan/SIOR/train_interactivate/interactive_category_1.json'
    ...
    region_mask_type: Optional[str] = field(default="point_visual_prompt_mask")#'box_visual_prompt_mask||scribble_visual_prompt_mask||point_visual_prompt_mask'
...
def compute_metric(intersection_meter,union_meter,acc_iou_meter, results_list):
    ...
    for results in results_list:
        ...
        # print(f"shape of topk_preds:{topk_preds.shape}")
...
def evaluation():
    ...
    with torch.no_grad():
        for idx, inputs in tqdm(enumerate(eval_dataloader), total=len(eval_dataloader)):
            annotations = gt_data[idx]['annotations1']
            h, w = gt_data[idx]['image1_info']['height'], gt_data[idx]['image1_info']['width']
            # generate gt mask
            masks = []
            ...
            # print(f"len gt_mask:{len(gt_mask)}")
    ...
    msg = "benchmark: {}: giou: {:.4f}, ciou: {:.4f}, miou: {:.4f}".format(save_suffix, giou, ciou, miou)
    print(msg)
