...
def intersectionAndUnionGPU(output, target, K, ignore_index=255):
    ...
    return area_intersection, area_union, area_output, area_target  # 新增返回area_output和area_target
...
class DataArguments:
    ...
    model_path: Optional[str] = field(default="/home/zhangkunquan/code/PSALM/output/checkpoint/RRSIS-D_SIOR_10")
    ...
    region_mask_type: Optional[str] = field(default="box_visual_prompt_mask")#'box_visual_prompt_mask||scribble_visual_prompt_mask||point_visual_prompt_mask'
...
def compute_metric(intersection_meter, union_meter, acc_iou_meter, tp_meter, fp_meter, fn_meter, results_list):  # 新增3个meter参数
    pred_list = []
    gt_list = []
    ...
    for results in results_list:
        ...
        max_intersection = 0
        max_union = 0
        max_area_output = 0  # 新增：存储预测区域面积
        max_area_target = 0  # 新增：存储GT区域面积
        max_i = 0
        # here topk=1, len(topk_preds)=1
        for i,pred_ in enumerate(topk_preds):
            # 新增接收area_output和area_target
            intersection, union, area_output, area_target = intersectionAndUnionGPU(
                torch.tensor(pred_).int().cuda().contiguous().clone(), torch.tensor(gt).int().cuda().contiguous(), 2, ignore_index=255
            )
            intersection, union = intersection.cpu().numpy(), union.cpu().numpy()
            area_output, area_target = area_output.cpu().numpy(), area_target.cpu().numpy()  # 转为numpy
            ...
            if fore_acc_iou > max_acc_iou:
                ...
                max_union = union
                max_area_output = area_output  # 记录最佳预测的面积
                max_area_target = area_target  # 记录对应的GT面积
                max_i = i
        # 计算TP、FP、FN（仅前景类，索引1）
        tp = max_intersection[1]  # 预测对的目标区域（交集）
        fp = max_area_output[1] - tp  # 预测为目标但实际是背景（假阳性）
        fn = max_area_target[1] - tp  # 实际是目标但未预测到（假阴性）
        # 更新所有meter
        intersection_meter.update(max_intersection)
        union_meter.update(max_union)
        acc_iou_meter.update(max_iou, n=1)
        tp_meter.update(tp)  # 新增更新TP
        fp_meter.update(fp)  # 新增更新FP
        fn_meter.update(fn)  # 新增更新FN
        # 保存预测和GT
...
def evaluation():
    ...
    # 新增3个meter：统计TP、FP、FN（用于计算精确率、召回率、F1）
    tp_meter = AverageMeter("TP", ":6.3f", Summary.SUM)
    fp_meter = AverageMeter("FP", ":6.3f", Summary.SUM)
    fn_meter = AverageMeter("FN", ":6.3f", Summary.SUM)
    ...
    with torch.no_grad():
        for idx, inputs in tqdm(enumerate(eval_dataloader), total=len(eval_dataloader)):
            ...
            pred, gt_mask = compute_metric(intersection_meter, union_meter, acc_iou_meter, tp_meter, fp_meter, fn_meter, cur_res)
    ...
    # 新增：计算精确率、召回率、F1（仅前景类）
    total_tp = tp_meter.sum
    total_fp = fp_meter.sum
    total_fn = fn_meter.sum
    precision = total_tp / (total_tp + total_fp + 1e-5)  # 精确率：预测为目标且正确的比例
    recall = total_tp / (total_tp + total_fn + 1e-5)     # 召回率：实际为目标且被预测到的比例
    f1 = 2 * precision * recall / (precision + recall + 1e-5)  # F1：精确率和召回率的调和平均
    # 更新输出信息
    msg = "benchmark: {}: giou: {:.4f}, ciou: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}".format(
        save_suffix, giou, ciou, precision, recall, f1
    )
    print(msg)
    # 保存结果（包含新指标）
