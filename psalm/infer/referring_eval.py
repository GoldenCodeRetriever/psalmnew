...
def compute_metric(intersection_meter, union_meter, acc_iou_meter, precision_meter, recall_meter, f1_meter, gt_cls, results_list):
    pred_list = []
    gt_list = []
    ...
    for results in results_list:
        ...
        # 计算精确率、召回率、F1（仅新增这部分，无其他修改）
        best_pred = topk_preds[max_i]
        true_pos = np.sum((best_pred == 1) & (gt == 1))
        false_pos = np.sum((best_pred == 1) & (gt == 0))
        false_neg = np.sum((best_pred == 0) & (gt == 1))
...
class DataArguments:
    ...
    referring_image_folder: Optional[str] = field(default='/nfs-data1/zhangkunquan/RRSIS-D_split/referring/test/JPEGImages')
    ...
    json_path: str = '/nfs-data1/zhangkunquan/RRSIS-D_split/referring/test/referring.json'
...
def evaluation():
    ...
    with torch.no_grad():
        for idx, inputs in tqdm(enumerate(eval_dataloader), total=len(eval_dataloader)):
            ...
            # 传入新增的三个meter（仅修改这一行的参数）
            pred,gt_mask = compute_metric(intersection_meter, union_meter, acc_iou_meter, precision_meter, recall_meter, f1_meter, gt_cls, cur_res)
    ...
    msg = "benchmark: {}: giou: {:.4f}, ciou: {:.4f}, precision: {:.4f}, recall: {:.4f}, f1: {:.4f}".format(
        save_suffix, giou, ciou, avg_precision, avg_recall, avg_f1
    )
    print(msg)
