...
class COCOInstanceNewBaselineDatasetMapper:
    ...
    def preprocess(self, dataset_dict, region_mask_type=None, mask_format='polygon'):
        ...
        if "file_name1" in dataset_dict:
            if isinstance(dataset_dict["file_name1"], str):
                image1 = utils.read_image(dataset_dict["file_name1"], format='RGB')
            else:
                image1 = np.array(dataset_dict["file_name1"])
            
            padding_mask1 = np.ones(image1.shape[:2])
            image1, transforms1 = T.apply_transform_gens(self.tfm_gens, image1)
            
            padding_mask1 = transforms1.apply_segmentation(padding_mask1)
            padding_mask1 = ~padding_mask1.astype(bool)
            
            image_shape1 = image1.shape[:2]
            
            image1 = torch.as_tensor(np.ascontiguousarray(image1.transpose(2, 0, 1)))
            dataset_dict["image1"] = (image1 - self.pixel_mean) / self.pixel_std
            dataset_dict["padding_mask1"] = torch.as_tensor(np.ascontiguousarray(padding_mask1))
            dataset_dict['transforms1'] = transforms1
        ...
        if "annotations" in dataset_dict:
            ...
            if len(annos) ==0:
                image_id = dataset_dict.get("image_id", "unknown_id")
                file_name = dataset_dict.get("file_name", "unknown_file")
                print(f"Error: annos is empty! image_id={image_id}, file_name={file_name}")
                print('error')
            ...
            if region_masks:
                ...
                if len(region_masks) == 0:
                    print(f"----------------The number of region masks ({len(region_masks)})-----------the number of instances ({len(instances)}) image: {dataset_dict.get('file_name')} ")
                    print(f"Empty region_masks after filtering for image {dataset_dict.get('file_name')}")
                assert len(region_masks) == len(instances), f"The number of region masks ({len(region_masks)}) must match the number of instances ({len(instances)})!!!! image: {dataset_dict.get('file_name')}"
            ...
            if 'vp_image' in dataset_dict:
                # print(f"掩码数量：{len(vp_region_masks)} 个")
                # for idx, mask in enumerate(vp_region_masks):
                #     try:
                #         mask_arr = np.asarray(mask)
                #         print(f"第{idx}个掩码：形状={mask_arr.shape}，维度={mask_arr.ndim}")
                #     except:
                #         print(f"第{idx}个掩码：无法转换为数组（可能是多边形/RLE格式）")
                # print("="*50 + "\n")
                # print(f"所有键：{list(dataset_dict.keys())}")  
                # print(f" 'file_name'（当前帧图像路径）：{dataset_dict.get('file_name', '不存在')}")
                # print(f" 'image_id'（当前帧ID）：{dataset_dict.get('image_id', '不存在')}")
                # print(f" 'height'（图像高度）：{dataset_dict.get('height', '不存在')}")
                # print(f" 'width'（图像宽度）：{dataset_dict.get('width', '不存在')}")
                # print(f" 'vp_image'（参考图信息）：{type(dataset_dict['vp_image'])}，内容预览：{str(dataset_dict['vp_image'])[:100]}") 
        ...
        if "annotations1" in dataset_dict and "file_name1" in dataset_dict:
   
            for anno in dataset_dict["annotations1"]:
                anno.pop("keypoints", None)

    
            annos1 = [
                utils.transform_instance_annotations(obj, transforms1, image_shape1)
                for obj in dataset_dict.pop("annotations1")
                if obj.get("iscrowd", 0) == 0
            ]
            if len(annos1) == 0:
                print('error: no annotations1 after transform')

            instances1 = utils.annotations_to_instances(annos1, image_shape1, mask_format=mask_format)
            
            instances1.gt_boxes = instances1.gt_masks.get_bounding_boxes()

            non_empty_instance_mask1 = [len(obj.get('segmentation', [])) > 0 for obj in annos1]
            instances1 = utils.filter_empty_instances(instances1)
            

            h1, w1 = instances1.image_size
            if hasattr(instances1, 'gt_masks'):
                gt_masks1 = instances1.gt_masks
                if hasattr(gt_masks1,'polygons'):
                    gt_masks1 = convert_coco_poly_to_mask(gt_masks1.polygons, h1, w1)
                else:
                    gt_masks1 = gt_masks1.tensor.to(dtype=torch.uint8)
                instances1.gt_masks = gt_masks1

            dataset_dict["instances1"] = instances1
