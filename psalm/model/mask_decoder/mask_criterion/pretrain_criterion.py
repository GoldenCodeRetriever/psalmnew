...
class PSALM_criterion(nn.Module):
    ...
    def __init__(self, matcher, losses, num_points, oversample_ratio, importance_sample_ratio, device):
        ...
        self.losses = losses # 需要计算的损失列表
        self.num_points = num_points # 点采样的总点数,用于掩码损失
        self.oversample_ratio = oversample_ratio # 过采样比例,增加采样多样性
        self.importance_sample_ratio = importance_sample_ratio # 重要性采样比例,优先采不确定区域
        self.device = device
...
class hungarian_matcher_PSALM(HungarianMatcher):
    ...
    def memory_efficient_forward(self, outputs, targets):
        ...
        bs, num_queries = outputs["pred_masks"].shape[:2] #[batch_size, num_queries, H_pred, W_pred]
        ...
        for b in range(bs):
            ...
            # [0.1, 0.2, 0.3, 0.4, 0.0],  # 第0个预测：对5个类别的概率
            # [0.5, 0.3, 0.1, 0.0, 0.1],  # 第1个预测
            # [0.0, 0.0, 0.5, 0.5, 0.0]   # 第2个预测
            # ])
            if 'pred_class_name_logits' in outputs and outputs['pred_class_name_logits'] is not None:
                class_prob = outputs['pred_class_name_logits'][b] # 取第b个样本的类别logits：[num_queries, num_classes]
        ...
        return [
            ...
            for i, j in indices #(src_indices, tgt_indices) 
        ]
    # i = torch.tensor([0, 2])
    # j = torch.tensor([1, 0])
