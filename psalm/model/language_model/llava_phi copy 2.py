...
class PSALM(PhiForCausalLM, LlavaMetaForCausalLM):
    ...
    def forward(
            ...
            dataset_type=None,
            prompt_images: Optional[torch.FloatTensor] = None,  # 提示图像
            prompt_info: Optional[List[Dict]] = None,  # 提示信息（如掩码、框：List[{'mask': ..., 'box': ...}]）
    ) -> Union[Tuple, CausalLMOutputWithPast]: 
        # 确保批次内所有样本的数据集类型一致,避免混合不同任务数据
        ...
        if seg_query_mask is not None:
            ...
            # 提取图像多尺度特征，输出 res2/res3/res4/res5 特征
            image_features = self.get_vision_tower_feature(images)
            # 像素解码器处理图像特征，输出掩码特征、Transformer 编码器特征、多尺度特征
            mask_features, transformer_encoder_features, multi_scale_features = self.pixel_decoder.forward_features(
                image_features)

            # 处理指代嵌入（SEG_embedding），从 hidden_states 提取指代相关特征，投影到目标维度
            if refer_embedding_indices is not None:
